...
import styles from './ExamplesButton.module.scss';
...
function ExamplesButton({setQueryStringInputValue, updateQueryString }) {
    ...
    <div key={example.text.toString()} className={styles.exempleRequestLine}>
      <Tooltip content="Essayez cette requête">
        <span className={styles.exampleRequest}>
          <FontAwesomeIcon icon={faSearch} size="xl" onClick={() => onClickExample(example)} />
        </span></Tooltip>
      {example.text}
    </div>
  );
    ...
    <>
      <div role="none" onClick={openModal} >
        <div className="flex items-center font-medium mr-2 w-32" id="headlessui-radiogroup-option-7" role="radio" aria-checked="false" tabIndex="-1">
          <span className="border-[1px] w-full text-center border-[#458ca5] text-[#458ca5] p-2">exemples</span>
        </div>
      </div>
